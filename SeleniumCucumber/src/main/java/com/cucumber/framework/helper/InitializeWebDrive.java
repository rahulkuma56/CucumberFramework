...
import com.cucumber.framework.configuration.browser.EdgeBrowser;
...
public class InitializeWebDrive {
	...
	public WebDriver standAloneStepUp(BrowserType bType) throws Exception {
		try {
			...
			case Edge:
				...
				return edge.getEdgeDriver(edge.getEdgeCapabilities());
	...
	@Before({"~@firefox","~@chrome","~@phantomjs","~@iexplorer","~@edge"})
	public void before() throws Exception {
		setUpDriver(ObjectRepo.reader.getBrowser());
		oLog.info(ObjectRepo.reader.getBrowser());
	}

	@After({"~@firefox","~@chrome","~@phantomjs","~@iexplorer","~@edge"})
	...
	@Before(order=2,value={"@edge"})
	public void beforeEdge() throws Exception {
		setUpDriver(BrowserType.Edge);
		oLog.info(BrowserType.Edge);
	}

	@After(order=2,value={"@edge"})
	public void afterEdge(Scenario scenario) throws Exception {
		tearDownDriver(scenario);
		oLog.info("");
	}
